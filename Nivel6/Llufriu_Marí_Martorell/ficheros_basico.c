...
#define DEBUGGER4 1 //Debug del nivel 4
#define DEBUGGER6 1 //Debug del nivel 6
 ...
 * nblogico: nº de bloque lógico
 * nivel_punteros: permisos del inodo
    ...
    if(ptr == 0) { //no existe bloque de datos
        ...
        if(reservar == 0) { //error lectura ∄ bloque
            ...
            if(nRangoBL == 0) { //si era un puntero Directo 
                ...
                #if DEBUGGER4
                    printf("[traducir_bloque_inodo()→ punteros_nivel%i[%i] = %i (reservado BF %i para BL %i)]\n", nivel_punteros+1, indice, ptr, ptr, nblogico);
                #endif
